...
func Init(d *cod.Cod) {
	...
	// 书籍ID
	bookIDReg := regexp.MustCompile(`^[1-9][0-9]{0,4}$`)
	d.AddValidator("bookID", func(value string) error {
		if !bookIDReg.MatchString(value) {
			return hes.New("book id should be numbers")
		}
		return nil
	})
